...
function Index({ auth, annees, queryParams = null, success }) {
    ...
    const searchFieldChanged = (type, value) => {
        ...
        router.get(route("annee.index"), queryParams);
    };
    ...
    const openModal = (mode, annee = null) => {
        setModalMode(mode);
    ...
    const validateForm = () => {
        ...
        if (!data.niveau) {
            errors.niveau = 'Le champ niveau est obligatoire.';
        }
    ...
    const handleFormSubmit = (event) => {
        ...
        const routeName = modalMode === 'add' ? 'annee.store' : 'annee.update';
        const action = modalMode === 'add' ? post : put;

        action(route(routeName, currentAnnee ? currentAnnee.id : null), data);
        closeModal();
    };
    ...
    const deleteAnnee = (annee) => {
        ...
        router.delete(route('annee.destroy', annee.id));
    };
    ...
    const sortedAnnees = annees.data.sort((a, b) => {
    ...
    const filteredAnnees = sortedAnnees.filter((annee) =>
        annee.id.toString().includes(searchQuery) ||
        annee.nom.toLowerCase().includes(searchQuery.toLowerCase()) ||
        annee.niveau.toLowerCase().includes(searchQuery.toLowerCase())
    );
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6 text-gray-900 dark:text-gray-100">
                                ...
                                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                        <tr>
                                            ...
                                            <th className='px-4 py-3 cursor-pointer' onClick={() => handleSort('nom')}>
                                                Nom {sortConfig.key === 'nom' && (
                                                    <FontAwesomeIcon icon={sortConfig.direction === 'asc' ? faSortUp : faSortDown} />
                                                )}
                                            </th>
                                            <th className='px-4 py-3 cursor-pointer' onClick={() => handleSort('niveau')}>
                                                Niveau {sortConfig.key === 'niveau' && (
                                                    <FontAwesomeIcon icon={sortConfig.direction === 'asc' ? faSortUp : faSortDown} />
                                                )}
                                            </th>
                                    ...
                                    <tbody>
                                        {filteredAnnees.map((annee) => (
                                            <tr key={annee.id} className='bg-white border-b dark:bg-gray-800 dark:border-gray-700'>
                                                <td className='px-4 py-3'>{annee.id}</td>
                                                <td className='px-4 py-3'>{annee.nom}</td>
                                                <td className='px-4 py-3'>{annee.niveau}</td>
                            ...
                            <Pagination links={annees.meta.links} />
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <form onSubmit={handleFormSubmit} className='p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg'>
                            <div className='mt-4'>
                                ...
                                {validationErrors.nom && (
                                    <div className='text-red-500 mt-2'>{validationErrors.nom}</div>
                                )}
                                <InputError message={errors.nom} className='mt-2' />
                            </div>

                            <div className='mt-4'>
                                <InputLabel htmlFor='niveau' value='Niveau' />
                                <TextInput
                                    type="text"
                                    name="niveau"
                                    id="niveau"
                                ...
                                {validationErrors.niveau && (
                                    <div className='text-red-500 mt-2'>{validationErrors.niveau}</div>
                                )}
                                <InputError message={errors.niveau} className='mt-2' />
                            </div>
