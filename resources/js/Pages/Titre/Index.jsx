...
function Index({ auth, titres, queryParams = null, success }) {
    ...
    const searchFieldChanged = (type, value) => {
        ...
        router.get(route("titre.index"), queryParams);
    };
    ...
    const openModal = (mode, titre = null) => {
        setModalMode(mode);
        setCurrentTitre(titre);
        if (mode === 'edit' && titre) {
            setData({ nom: titre.nom });
        } else {
            setData({ nom: '' });
        }
        setValidationErrors({});
    ...
    const handleFormSubmit = (event) => {
        ...
        const routeName = modalMode === 'add' ? 'titre.store' : 'titre.update';
        ...
        if (modalMode === 'add') {
            ...
            action(route(routeName, currentTitre.id), {
                ...data,
                onSuccess: () => {
                    closeModal();
                }
    ...
    const deleteTitre = (titre) => {
        if (!confirm('Êtes-vous sûr de vouloir supprimer ce titre?')) {
            return;
        }
        router.delete(route('titre.destroy', titre.id));
    };
    ...
    const sortedTitres = titres.data.sort((a, b) => {
    ...
    const filteredTitres = sortedTitres.filter((titre) =>
        titre.id.toString().includes(searchQuery) ||
        titre.nom.toLowerCase().includes(searchQuery.toLowerCase())
    );
                        ...
                        <div className="p-6 text-gray-900 dark:text-gray-100">
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    ...
                                    <tbody>
                                        {filteredTitres.map((titre) => (
                                            <tr key={titre.id} className='bg-white border-b dark:bg-gray-800 dark:border-gray-700'>
                                                <td className='px-4 py-3'>{titre.id}</td>
                                                <td className='px-4 py-3'>{titre.nom}</td>
                                                <td className='px-4 py-3 text-right flex justify-end'>
                                                    <button
                                                        onClick={() => openModal('edit', titre)}
                                                        className='text-blue-600 dark:text-blue-500 mx-1'
                                                    >
                                                        <FontAwesomeIcon icon={faEdit} />
                                                    </button>
                                                    <button
                                                        onClick={() => deleteTitre(titre)}
                            ...
                            <Pagination links={titres.meta.links} />
                        </div>
                    </div>
                </div>
            </div>
